 ...
 * you add the slots to your gui. It can also be used to sync server side data
 * with the client but
 ...
 * For this example, we only need markDirty() and isUsableByPlayer(). I've
 * chosen to implement these as callback functions
 * (lambdas) because I think it's clearer than providing an
 * Optional<TileEntityBasic>, but it could easily be done that
 * way as well, because the functions are only needed on the server side, when
 * the TileEntity is available.
 * On the client side, there is no TileEntity available.
 */
public class DebarkerContainer extends Container {

	public static DebarkerContainer createContainerServerSide(int windowID, PlayerInventory playerInventory,
			DebarkerContents chestContents) {
		return new DebarkerContainer(windowID, playerInventory, chestContents);
	}

	public static DebarkerContainer createContainerClientSide(int windowID, PlayerInventory playerInventory,
			net.minecraft.network.PacketBuffer extraData) {

		DebarkerContents chestContents = DebarkerContents
				.createForClientSideContainer(DebarkerTileEntity.NUMBER_OF_SLOTS);
	...
	private static final int TE_INVENTORY_SLOT_COUNT = DebarkerTileEntity.NUMBER_OF_SLOTS; // must match
