package com.csykes.allthesmallthings.menus.chipper;
...
public class ChipperMenu extends AbstractContainerMenu {
    public final ChipperBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;
    public ChipperMenu(int id, Inventory playerInventory, FriendlyByteBuf extraData) {
        this(id, playerInventory, playerInventory.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public ChipperMenu(int id, Inventory playerInventory, BlockEntity entity, ContainerData data) {
        super(_ModMenus.CHIPPER.get(),id);
        checkContainerSize(playerInventory, 3);
        blockEntity = (ChipperBlockEntity) entity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new InputSlotItemHandler(handler, 0, 152, 10));
            this.addSlot(new SlotItemHandler(handler, 1, 152, 62));
        });
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, _ModBlocks.MASON_CHIPPER.get());
    }
